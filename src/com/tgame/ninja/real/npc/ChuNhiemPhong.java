...
public class ChuNhiemPhong implements INpcHandler {
    ...
    public void openMenu(Player player, PlayerTemplate playerTemplate) {
        ...
        if ((player.classId == 5 || player.classId == 6) && player.taskMain == null && (player.getTaskFinish() == 9 || player.getTaskFinish() == 10 || player.getTaskFinish() == 11 || player.getTaskFinish() == 12)) {
        ...
        } else if ((player.classId == 5 || player.classId == 6) && player.taskMain != null && player.taskMain.index >= player.taskMain.template.subNames[player.getSession().typeLanguage].length - 1 && (player.getTaskFinish() == 9 || player.getTaskFinish() == 10 || player.getTaskFinish() == 11 || player.getTaskFinish() == 12)) {
            if (playerTemplate.menuId == 0 && playerTemplate.optionId == 0) {
                player.doTaskFinish(playerTemplate.playerTemplateId);
                return;
            }
            --playerTemplate.menuId;
        } else if (player.taskMain != null && player.getTaskFinish() == 13 && player.taskMain.index == 2) {
            if (playerTemplate.menuId == 0) {
                int time5 = 300;
                Dun dun = Dun.createDun(0);
                if (dun == null) {
        ...
        if (playerTemplate.menuId == 0) {
            ...
            if (playerTemplate.optionId == 0) {
                playerMain.classId = 5;
                playerMain.p_strength = 15;
                playerMain.p_agile = 5;
                playerMain.p_mp = 5;
                playerMain.p_hp = 5;
                player.updateClass();
                Item itemBook = new Item(76, player.playerId, true, "char 12");
                player.addItem(itemBook, 3, index2);
                player.sendAddItemBag(itemBook);
                Item itemWeapon = new Item(104, player.playerId, true, "char 13");
                itemWeapon.sys = 3;
                ...
                player.sendOpenUISay(playerTemplate.playerTemplateId, AlertFunction.INPUT_CLASS3(player.getSession()));
            } else if (playerTemplate.optionId == 1) {
                playerMain.classId = 6;
                playerMain.p_strength = 5;
                playerMain.p_agile = 5;
                playerMain.p_mp = 10;
                playerMain.p_hp = 10;
                player.updateClass();
                Item itemBook = new Item(85, player.playerId, true, "char 14");
                player.addItem(itemBook, 3, index2);
                player.sendAddItemBag(itemBook);
                Item itemWeapon = new Item(119, player.playerId, true, "char 15");
                itemWeapon.sys = 6;
                itemWeapon.options = Item.getOptionTemplate(itemWeapon);
                if (itemWeapon.options != null) {
                    itemWeapon.options.remove(5);
            ...
            if (playerTemplate.optionId == 0) {
                if (playerMain.classId == 5 || playerMain.classId == 6) {
                    if (playerMain.resetPotential > 0) {
                        ...
                        playerMain.resetPotential();
                        if (playerMain.resetPotential > 0) {
                            player.sendOpenUISay(playerTemplate.playerTemplateId, NJUtil.replace(AlertFunction.FREE_POTENTIAL_1(player.getSession()), String.valueOf(player.resetPotential)));
                        } else {
                            player.sendOpenUISay(playerTemplate.playerTemplateId, NJUtil.replace(AlertFunction.FREE_POTENTIAL_2(player.getSession()), String.valueOf(player.resetPotential)));
                        }
                    } else {
            ...
            if (playerTemplate.optionId == 0) {
                if (player.taskMain != null && player.taskMain.template.taskId == 8 && player.taskMain.index == 3) {
                    player.doTaskNext();
