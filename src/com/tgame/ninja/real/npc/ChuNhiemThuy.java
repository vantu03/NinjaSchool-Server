...
public class ChuNhiemThuy implements INpcHandler {
    ...
    public void openMenu(Player player, PlayerTemplate playerTemplate) {
        ...
        if ((player.classId == 3 || player.classId == 4) && player.taskMain == null && (player.getTaskFinish() == 9 || player.getTaskFinish() == 10 || player.getTaskFinish() == 11 || player.getTaskFinish() == 12)) {
        ...
        } else if ((player.classId == 3 || player.classId == 4) && player.taskMain != null && player.taskMain.index >= player.taskMain.template.subNames[player.getSession().typeLanguage].length - 1 && (player.getTaskFinish() == 9 || player.getTaskFinish() == 10 || player.getTaskFinish() == 11 || player.getTaskFinish() == 12)) {
            if (playerMain.isNhanban()) {
                player.sendOpenUISay(playerTemplate.playerTemplateId, AlertFunction.PHAN_THAN_NO_PERMIS(player.getSession()));
                return;
            }
            ...
            --playerTemplate.menuId;
        } else if (player.taskMain != null && player.getTaskFinish() == 13 && player.taskMain.index == 1) {
            if (playerMain.isNhanban()) {
                player.sendOpenUISay(playerTemplate.playerTemplateId, AlertFunction.PHAN_THAN_NO_PERMIS(player.getSession()));
                return;
            }
        ...
        if (playerTemplate.menuId == 0) {
            ...
            if (playerTemplate.optionId == 0) {
                playerMain.classId = 3;
                playerMain.p_strength = 15;
                playerMain.p_agile = 5;
                playerMain.p_mp = 5;
                playerMain.p_hp = 5;
                player.updateClass();
                Item itemBook = new Item(58, player.playerId, true, "char 8");
                player.addItem(itemBook, 3, index2);
                player.sendAddItemBag(itemBook);
                Item itemWeapon = new Item(99, player.playerId, true, "char 9");
                itemWeapon.sys = 2;
                ...
                player.sendOpenUISay(playerTemplate.playerTemplateId, AlertFunction.INPUT_CLASS2(player.getSession()));
            } else if (playerTemplate.optionId == 1) {
                playerMain.classId = 4;
                playerMain.p_strength = 5;
                playerMain.p_agile = 5;
                playerMain.p_mp = 10;
                playerMain.p_hp = 10;
                player.updateClass();
                Item itemBook = new Item(67, player.playerId, true, "char 10");
                player.addItem(itemBook, 3, index2);
                player.sendAddItemBag(itemBook);
                Item itemWeapon = new Item(109, player.playerId, true, "char 11");
                itemWeapon.sys = 5;
                itemWeapon.options = Item.getOptionTemplate(itemWeapon);
                if (itemWeapon.options != null) {
                    itemWeapon.options.remove(5);
            ...
            if (playerTemplate.optionId == 0) {
                if (player.taskMain != null && player.taskMain.template.taskId == 8 && player.taskMain.index == 2) {
                    player.doTaskNext();
